 ...
 *		<tr><td><b>Package</b></td> <td><a href = "https://github.com/lakshmik/DROP/tree/master/src/main/java/org/drip/sample/treasuryfuturesrisk/README.md">Treasury Futures Key Rate Duration</a></td></tr>
 *  </table>
 *	<br>
 *
...
public class CN1KeyRateDuration
{
		...
		throws Exception
	{
		...
		double[] yield02YArray = treasuryMarkCSVGrid.doubleArrayAtColumn (1);

		double[] yield03YArray = treasuryMarkCSVGrid.doubleArrayAtColumn (2);

		double[] yield04YArray = treasuryMarkCSVGrid.doubleArrayAtColumn (3);

		double[] yield05YArray = treasuryMarkCSVGrid.doubleArrayAtColumn (4);

		double[] yield07YArray = treasuryMarkCSVGrid.doubleArrayAtColumn (5);

		double[] yield10YArray = treasuryMarkCSVGrid.doubleArrayAtColumn (6);

		double[] yield20YArray = treasuryMarkCSVGrid.doubleArrayAtColumn (7);

		double[] yield30YArray = treasuryMarkCSVGrid.doubleArrayAtColumn (8);
			...
			++treasuryMarkIndex)
		{
			treasuryMarkMap02Y.put (
				...
				yield02YArray[treasuryMarkIndex]
			);

			treasuryMarkMap03Y.put (
				treasuryMarkDateArray[treasuryMarkIndex],
				yield03YArray[treasuryMarkIndex]
			);

			treasuryMarkMap04Y.put (
				treasuryMarkDateArray[treasuryMarkIndex],
				yield04YArray[treasuryMarkIndex]
			);

			treasuryMarkMap05Y.put (
				treasuryMarkDateArray[treasuryMarkIndex],
				yield05YArray[treasuryMarkIndex]
			);

			treasuryMarkMap07Y.put (
				treasuryMarkDateArray[treasuryMarkIndex],
				yield07YArray[treasuryMarkIndex]
			);

			treasuryMarkMap10Y.put (
				treasuryMarkDateArray[treasuryMarkIndex],
				yield10YArray[treasuryMarkIndex]
			);

			treasuryMarkMap20Y.put (
				treasuryMarkDateArray[treasuryMarkIndex],
				yield20YArray[treasuryMarkIndex]
			);

			treasuryMarkMap30Y.put (
				treasuryMarkDateArray[treasuryMarkIndex],
				yield30YArray[treasuryMarkIndex]
			);
		}
		...
		for (TenorDurationNodeMetrics tenorDurationNodeMetrics : tenorDurationNodeMetricsList) {
			String tenorDurationNodeMetricsDump =
				...
				FormatUtil.FormatDouble (
					tenorDurationNodeMetrics.r1 ("SpotCTDCleanPrice"),
					1,
					5,
					100.
				) + "," +
				FormatUtil.FormatDouble (
					tenorDurationNodeMetrics.r1 ("ExpiryCTDCleanPrice"),
					1,
					5,
					100.
				) + "," +
			...
			for (Map.Entry<String, Double> tenorDurationNodeMetricEntry :
				tenorDurationNodeMetrics.krdMap().entrySet())
			{
				tenorDurationNodeMetricsDump +=
					"," + FormatUtil.FormatDouble (tenorDurationNodeMetricEntry.getValue(), 1, 4, 1.);
			}
