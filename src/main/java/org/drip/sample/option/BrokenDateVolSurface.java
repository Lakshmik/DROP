...
public class BrokenDateVolSurface
{
		...
		throws Exception
	{
		...
		for (String maturity : maturityTenorArray) {
		...
		for (double strikeATMFactor : strikeATMFactorArray) {
			...
			for (String maturity : maturityTenorArray) {
				System.out.print (
					"  " + FormatUtil.FormatDouble (volSurface.node (strikeATMFactor, maturity), 2, 2, 100.)
						+ "%"
				);
			}
		...
		strikeATMFactorArray = new double[]
		{
			0.850,
			0.925,
			1.000,
