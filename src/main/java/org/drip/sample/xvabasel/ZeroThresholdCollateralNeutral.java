...
public class ZeroThresholdCollateralNeutral
{
		...
		throws Exception
	{
		...
		for (int stepIndex = 0; stepIndex <= stepCount; ++stepIndex) {
			swapPortfolioValueRealizationArray[stepIndex] = stepIndex > maturityStepSize ? 0. :
				swapNotional * timeToMaturity * (maturityStepSize - stepIndex) *
		...
		throws Exception
	{
		...
		for (int stepIndex = 0; stepIndex <= stepCount; ++stepIndex) {
			...
			marketVertexArray[stepIndex] = MarketVertex.Nodal (
				...
				new MarketVertexEntity (
					...
					bankRecoveryRate,
					bankFundingSpread,
					Math.exp (-0.5 * bankHazardRate * (1. - bankRecoveryRate) * stepCount),
					Double.NaN,
					Double.NaN,
					Double.NaN
				),
		...
		for (int pathIndex = 0; pathIndex < pathCount; ++pathIndex) {
			...
			for (int stepIndex = 0; stepIndex <= stepCount; ++stepIndex) {
				...
				startDate = endDate;
				valueStart1 = portfolio1ValueGrid[pathIndex][stepIndex];
				valueStart2 = portfolio2ValueGrid[pathIndex][stepIndex];
			}
	...
	private static final void BaselAccountingMetrics (
		...
		throws Exception
	{
		OTCAccountingModus fcafbaOTCAccountingModus =
			new OTCAccountingModusFCAFBA (groundExposureAdjustmentAggregator);

		OTCAccountingModus fvafdaOTCAccountingModus =
			new OTCAccountingModusFVAFDA (groundExposureAdjustmentAggregator);

		OTCAccountingPolicy fcafbaOTCAccountingPolicy =
			fcafbaOTCAccountingModus.feePolicy (expandedExposureAdjustmentAggregator);

		OTCAccountingPolicy fvafdaOTCAccountingPolicy =
			fvafdaOTCAccountingModus.feePolicy (expandedExposureAdjustmentAggregator);
		...
		System.out.println (
			"\t|| FCA/FBA Accounting => " + FormatUtil.FormatDouble (
				fcafbaOTCAccountingModus.contraAssetAdjustment(),
				1,
				4,
				1.
			) + " | " + FormatUtil.FormatDouble (
				fcafbaOTCAccountingModus.contraLiabilityAdjustment(),
				1,
				4,
				1.
			) + " | " + FormatUtil.FormatDouble (
		...
		System.out.println (
			"\t|| FVA/FDA Accounting => " + FormatUtil.FormatDouble (
				fvafdaOTCAccountingModus.contraAssetAdjustment(),
				1,
				4,
				1.
			) + " | " + FormatUtil.FormatDouble (
				fvafdaOTCAccountingModus.contraLiabilityAdjustment(),
				1,
				4,
				1.
			) + " | " + FormatUtil.FormatDouble (
