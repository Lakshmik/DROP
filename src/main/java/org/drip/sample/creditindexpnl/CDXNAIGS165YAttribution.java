 ...
 *		<tr><td><b>Package</b></td> <td><a href = "https://github.com/lakshmik/DROP/tree/master/src/main/java/org/drip/sample/creditindexpnl/README.md">CDX NA IG PnL Attribution</a></td></tr>
 *  </table>
 *	<br>
 * 
...
public class CDXNAIGS165YAttribution
{
		...
		throws Exception
	{
		...
		for (int closeDateIndex = 0; closeDateIndex < closeDateCount; ++closeDateIndex) {
			...
			adblCreditIndexQuotedSpread[closeDateIndex] *= 10000.;
		}
		...
		for (PositionChangeComponents positionChangeComponents : positionChangeComponentsList) {
			...
			CDSMarketSnap t1CDSMarketSnap = (CDSMarketSnap) positionChangeComponents.t1PositionMarketSnap();

			CDSMarketSnap t2CDSMarketSnap = (CDSMarketSnap) positionChangeComponents.t2PositionMarketSnap();

			System.out.println (
				positionChangeComponents.t1() + ", " +
				positionChangeComponents.t2() + ", " +
				t1CDSMarketSnap.creditLabel() + ", " +
				horizonGap + "," +
				...
				FormatUtil.FormatDouble (t1CDSMarketSnap.fixedCoupon(), 1, 2, 10000.) + ", " +
				FormatUtil.FormatDouble (t1CDSMarketSnap.currentFairPremium(), 1, 4, 10000.) + ", " +
				FormatUtil.FormatDouble (t2CDSMarketSnap.currentFairPremium(), 1, 4, 10000.) + ", " +
				FormatUtil.FormatDouble (t1CDSMarketSnap.rollDownFairPremium(), 1, 4, 10000.) + ", " +
				FormatUtil.FormatDouble (t1CDSMarketSnap.cleanDV01(), 1, 4, 1.)
			);
		}
