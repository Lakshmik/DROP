...
public class ImpactExponentAnalysis
{
		...
		throws Exception
	{
		...
		System.out.println (
			"\t|                OPTIMAL MEASURES IMPACT EXPONENT DEPENDENCE                ||"
		);
		...
		System.out.println (
			"\t|            - Temporary Market Impact Exponent                             ||"
		);
		...
		for (double k : kArray) {
			...
			System.out.println (
				"\t|" + FormatUtil.FormatDouble (k, 1, 2, 1.) + " |" +
				FormatUtil.FormatDouble (
					almgren2003Estimator.breakevenPrincipalDiscount(),
					1,
					2,
					1.
				) + " | " + FormatUtil.FormatDouble (
