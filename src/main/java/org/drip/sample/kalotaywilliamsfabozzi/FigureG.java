...
public class FigureG
{
	...
	private static final Map<Double, List<Double>> ProjectedCallableBondValueListMapReconciler()
	{
		...
		projected2YCallableBondValueList.add (100.000);
		...
		projected3YCallableBondValueList.add (100.000);

		projected3YCallableBondValueList.add (99.732);
		...
		throws Exception
	{
		...
		System.out.println ("\t|   - Projected Proxy Callable Bond Price              ||");

		System.out.println ("\t|------------------------------------------------------||");

		for (double time : projectedCallableBondValueListMap.keySet()) {
			String dump = "\t|" + FormatUtil.FormatDouble (time, 1, 0, 1.) + " => ";
			...
			for (int i = 0; i < projectedCallableBondValueList.size(); ++i) {
				dump += FormatUtil.FormatDouble (
					projectedCallableBondValueList.get (i),
					3,
					3,
					100.,
					false
				) + " (" + FormatUtil.FormatDouble (
					projectedCallableBondValueReconcilerList.get (i),
					3,
					3,
					1.,
					false
