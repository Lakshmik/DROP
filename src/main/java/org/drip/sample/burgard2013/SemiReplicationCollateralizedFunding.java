...
public class SemiReplicationCollateralizedFunding
{
	...
	private static final double[] ATMSwapRateOffsetRealization (
		...
		final double atmSwapRateOffsetInitial,
		...
		throws Exception
	{
		...
		atmSwapRateOffsetArray[0] = atmSwapRateOffsetInitial;
		...
		JumpDiffusionEdge[] jumpDiffusionEdgeArray = atmSwapRateOffsetDiffusionEvolver.incrementSequence (
			new JumpDiffusionVertex (time, atmSwapRateOffsetInitial, 0., false),
		...
		throws Exception
	{
		...
		for (int stepIndex = 0; stepIndex <= stepCount; ++stepIndex) {
			...
			marketVertexArray[stepIndex] = MarketVertex.Nodal (
				adtVertex[stepIndex] = spotDate.addMonths (6 * stepIndex),
		...
		for (int pathIndex = 0; pathIndex < pathCount; ++pathIndex) {
			...
			aadblPortfolio2Value[pathIndex] = SwapPortfolioValueRealization (
				atmSwapRateOffsetDiffusionEvolver,
			...
			double startingValue1 = time * atmSwapRateOffsetStart1;
			double startingValue2 = time * atmSwapRateOffsetStart2;
			...
			for (int stepIndex = 0; stepIndex <= stepCount; ++stepIndex) {
				JulianDate endDate = adtVertex[stepIndex];
				double endingValue1 = portfolio1ValueGrid[pathIndex][stepIndex];
				double endingValue2 = aadblPortfolio2Value[pathIndex][stepIndex];

				if (0 != stepIndex) {
					collateralGroupVertex1Array[stepIndex] = BurgardKjaerBuilder.SemiReplicationDualBond (
						adtVertex[stepIndex],
					...
					collateralGroupVertex2Array[stepIndex] = BurgardKjaerBuilder.SemiReplicationDualBond (
						adtVertex[stepIndex],
						aadblPortfolio2Value[pathIndex][stepIndex],
						0.,
						new CollateralAmountEstimator (
							positionGroupSpecification,
							new BrokenDateInterpolatorLinearT (
					...
					collateralGroupVertex2Array[stepIndex] = BurgardKjaerBuilder.Initial (
						adtVertex[stepIndex],
						aadblPortfolio2Value[pathIndex][0],
						marketVertexArray[stepIndex],
						closeOut
					);
				}
				...
				startDate = endDate;
				startingValue1 = endingValue1;
				startingValue2 = endingValue2;
			}
	...
	private static final void BaselAccountingMetrics (
		...
		throws Exception
	{
		OTCAccountingModus otcAccountingModusFCAFBA = new OTCAccountingModusFCAFBA (eadGround);

		OTCAccountingModus otcAccountingModusFVAFDA = new OTCAccountingModusFVAFDA (eadGround);

		OTCAccountingPolicy fcafbaOTCAccountingPolicy = otcAccountingModusFCAFBA.feePolicy (eadExpanded);

		OTCAccountingPolicy fvafbaOTCAccountingPolicy = otcAccountingModusFVAFDA.feePolicy (eadExpanded);
