...
public class SetOffCollateralizedFundingStochastic
{
		...
		throws Exception
	{
		...
		for (int pathIndex = 0; pathIndex < pathCount; ++pathIndex) {
			...
			double valueStart1 = time * initialATMSwapRateOffset1;
			double valueStart2 = time * initialATMSwapRateOffset2;
			...
			for (int stepIndex = 0; stepIndex <= stepCount; ++stepIndex) {
				...
				if (0 != stepIndex) {
					collateralGroupVertex1Array[stepIndex] = BurgardKjaerBuilder.SetOff (
					...
					collateralGroupVertex2Array[stepIndex] = BurgardKjaerBuilder.SetOff (
						vertexDateArray[stepIndex],
			...
			extendedMonoPathExposureAdjustmentArray[pathIndex] = new MonoPathExposureAdjustment (
				new AlbaneseAndersenFundingGroupPath[] {
					new AlbaneseAndersenFundingGroupPath (
						new AlbaneseAndersenNettingGroupPath[] {
							new AlbaneseAndersenNettingGroupPath (extendedCollateralGroupPathArray, marketPath)
						},
						marketPath
					)
				}
		...
		throws Exception
	{
		...
		System.out.println (
			"\t|| Minimum => " + FormatUtil.FormatDouble (
				...
				fvaUnivariateDiscreteThin.minimum(), 2, 2, 1.
			) + "  | " + FormatUtil.FormatDouble (
