...
public class UncollateralizedCollateralReceivableStochastic
{
		...
		throws Exception
	{
		...
		int maturityStepSize = (int) (dblTimeMaturity / timeWidth);
		...
		for (int stepIndex = 0; stepIndex <= stepCount; ++stepIndex) {
			swapPortfolioValueRealizationArray[stepIndex] = stepIndex > maturityStepSize ? 0. :
				dblSwapNotional * timeWidth * (maturityStepSize - stepIndex) *
			...
			extendedMonoPathExposureAdjustmentArray[pathIndex] = new MonoPathExposureAdjustment (
				new AlbaneseAndersenFundingGroupPath[] {
					new AlbaneseAndersenFundingGroupPath (
						new AlbaneseAndersenNettingGroupPath[] {
							new AlbaneseAndersenNettingGroupPath (
								groundCollateralGroupPathArray,
								marketPath
							),
							new AlbaneseAndersenNettingGroupPath (
		...
		throws Exception
	{
		...
		OTCAccountingModus fvafdaAccountingModusFVAFDA =
		...
		OTCAccountingPolicy fvafdaOTCAccountingPolicy =
			fvafdaAccountingModusFVAFDA.feePolicy (expandedExposureAdjustmentAggregator);
		...
		System.out.println (
			"\t|| FVA/FDA Accounting => " + FormatUtil.FormatDouble (
				fvafdaAccountingModusFVAFDA.contraAssetAdjustment(),
				1,
				4,
				1.
			) + " | " + FormatUtil.FormatDouble (
				fvafdaAccountingModusFVAFDA.contraLiabilityAdjustment(),
				1,
				4,
				1.
			) + " | " + FormatUtil.FormatDouble (
