...
public class CMVReconciler2
{
		...
		throws Exception
	{
		...
		for (int assetIndex = 0; assetIndex < reconcilerAssetWeightsArray.length; ++assetIndex) {
		...
		for (int assetIndex = 0; assetIndex < optimalAssetComponentArray.length; ++assetIndex) {
			System.out.println (
				...
				FormatUtil.FormatDouble (optimalAssetComponentArray[assetIndex].amount(), 2, 2, 100.) +
					"% | " +
				FormatUtil.FormatDouble (reconcilerAssetComponentArray[assetIndex].amount(), 2, 2, 100.) +
					"% ||"
			);
		}
		...
		System.out.println (
			"\t| Optimal Portfolio Normalize          : " +
				FormatUtil.FormatDouble (optimizationOutput.optimalPortfolio().notional(), 1, 2, 1.) +
				"   ||"
		);

		System.out.println (
			"\t| Optimal Portfolio Input Return       : " +
				FormatUtil.FormatDouble (portfolioDesignReturn, 1, 2, 100.) + "%  ||"
		);

		System.out.println (
			"\t| Optimal Portfolio Expected Return    : " +
				FormatUtil.FormatDouble (optimizationOutput.optimalMetrics().excessReturnsMean(), 1, 2, 100.)
				+ "%  ||"
		);
