...
/**
 * <i>MichaelisMenten</i> implements the Michaelis-Menten Kinetics Model for an Enzyme-mediated Reaction. The
 * 	References are:
 *
 *  <br><br>
 *  <ul>
...
public class MichaelisMenten
	extends R1ToR1
{
	private double _kM = Double.NaN;
	private double _vMax = Double.NaN;

	/**
	 * <i>MichaelisMenten</i> Constructor
	 * 
	 * @param vMax V<sub>max</sub>
	 * @param kM k<sub>M</sub>
	 * 
		...
		throws Exception
	{
		...
		if (!NumberUtil.IsValid (_vMax = vMax) || !NumberUtil.IsValid (_kM = kM)) {
			throw new Exception ("MichaelisMenten Constructor => Invalid Inputs");
		}
	}

	/**
	 * Retrieve V<sub>max</sub>
	 * 
		...
		throws Exception
	{
		if (!NumberUtil.IsValid (x)) {
			throw new Exception ("MichaelisMenten::evaluate => Invalid Inputs");
		}

		return _vMax * x / (_kM + x);
	}
}
