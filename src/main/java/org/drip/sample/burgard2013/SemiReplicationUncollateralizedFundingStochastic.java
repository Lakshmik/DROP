...
public class SemiReplicationUncollateralizedFundingStochastic
{
	...
	private static final double[] ATMSwapRateOffsetRealization (
		...
		final double initiaATMSwapRateOffset,
		final double[] randomArray,
		final double time,
		final double timeWidth,
		final int stepCount)
		throws Exception
	{
		...
		atmSwapRateOffsetArray[0] = initiaATMSwapRateOffset;
		double[] timeWidthArray = new double[stepCount];
		...
		JumpDiffusionEdge[] jumpDiffusionEdgeArray = atmSwapRateOffsetDiffusionEvolver.incrementSequence (
			new JumpDiffusionVertex (time, initiaATMSwapRateOffset, 0., false),
			JumpDiffusionEdgeUnit.Diffusion (timeWidthArray, randomArray),
			timeWidth
		);
	...
	private static final double[] SwapPortfolioValueRealization (
		...
		final double startingATMSwapRate,
		final double[] randomArray,
		final int stepCount,
		final double time,
		final double timeWidth,
	...
	private static final ExposureAdjustmentAggregator[] Mix (
		...
		final double startingATMSwapRateOffset1,
		final double swapNotional1,
		final double timeToMaturity2,
		final double startingATMSwapRateOffset2,
		...
		throws Exception
	{
		...
		double[][] collateralBalanceArray = new double[pathCount][stepCount + 1];
		...
		MonoPathExposureAdjustment[] extendedMonoPathExposureAdjustmentArray =
			new MonoPathExposureAdjustment[pathCount];
		double bankSeniorFundingSpreadInitial =
			bankHazardRateInitial / (1. - bankSeniorRecoveryRateInitial);
		...
		for (int pathIndex = 0; pathIndex < pathCount; ++pathIndex) {
			double[][] numeraireArray = R1MatrixUtil.Transpose (Path (correlationMatrix, vertexCount));

			portfolio1ValueGrid[pathIndex] = SwapPortfolioValueRealization (
				atmSwapRateOffsetDiffusionEvolver,
				startingATMSwapRateOffset1,
				numeraireArray[0],
				vertexCount,
				time,
				timeWidth,
			...
			for (int stepIndex = 0; stepIndex <= stepCount; ++stepIndex) {
				...
				collateralBalanceArray[pathIndex][stepIndex] = 0.;
		...
		throws Exception
	{
		...
		System.out.println (
			"\t|| FCA/FBA Accounting => " + FormatUtil.FormatDouble (
				...
				oapFCAFBA.fundingTransferPricing(),
				3,
				0,
				10000.
			) + " | " + FormatUtil.FormatDouble (
				oapFCAFBA.cet1Change(),
				3,
				0,
				10000.
			) + " | " + FormatUtil.FormatDouble (
				oapFCAFBA.contraLiabilityChange(),
				3,
				0,
				10000.
			) + " | " + FormatUtil.FormatDouble (
				oapFCAFBA.portfolioValueChange(),
				3,
				0,
				10000.
			) + " || "
		...
		System.out.println (
			"\t|| FVA/FDA Accounting => " + FormatUtil.FormatDouble (
				...
				oapFVAFDA.fundingTransferPricing(),
				3,
				0,
				10000.
			) + " | " + FormatUtil.FormatDouble (
				oapFVAFDA.cet1Change(),
				3,
				0,
				10000.
			) + " | " + FormatUtil.FormatDouble (
				oapFVAFDA.contraLiabilityChange(),
				3,
				0,
				10000.
			) + " | " + FormatUtil.FormatDouble (
				oapFVAFDA.portfolioValueChange(),
				3,
				0,
				10000.
			) + " || "
