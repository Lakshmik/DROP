...
public class BlackHestonForwardOption
{
	...
	private static final CalibratableComponent[] DepositInstrumentsFromMaturityDays (
		...
		final int[] depositMaturityDayArray,
		final int futuresCount,
		final String currency)
		throws Exception
	{
		CalibratableComponent[] calibratableComponentArray =
			new CalibratableComponent[depositMaturityDayArray.length + futuresCount];

		for (int depositIndex = 0; depositIndex < depositMaturityDayArray.length; ++depositIndex) {
			calibratableComponentArray[depositIndex] = SingleStreamComponentBuilder.Deposit (
				effectiveDate,
				effectiveDate.addBusDays (depositMaturityDayArray[depositIndex], currency),
		...
		for (int futuresIndex = depositMaturityDayArray.length;
			futuresIndex < depositMaturityDayArray.length + futuresCount;
			++futuresIndex)
		{
			calibratableComponentArray[futuresIndex] =
				futuresArray[futuresIndex - depositMaturityDayArray.length];
		}
		...
		throws Exception
	{
		...
		for (int irsIndex = 0; irsIndex < maturityTenorArray.length; ++irsIndex) {
			...
			irs.setPrimaryCode ("IRS." + maturityTenorArray[irsIndex] + "." + currency);

			irsArray[irsIndex] = irs;
		}
