...
public class FixFloatSwapAnalysis
{
	...
	private static final double RunWithVolCorrelation (
		...
		throws Exception
	{
		int valuationDateJulian = valuationParams.valueDate();

		String forwardLabelCurrency = forwardLabel.currency();

		FundingLabel fundingLabel = FundingLabel.Standard (forwardLabelCurrency);

		curveSurfaceQuoteContainer.setForwardVolatility (
			ScenarioDeterministicVolatilityBuilder.FlatForward (
				valuationDateJulian,
				VolatilityLabel.Standard (forwardLabel),
				forwardLabelCurrency,
				forwardVolatility
			)
		);
		...
		System.out.println (
			"\t|| [" + FormatUtil.FormatDouble (fundingVolatility, 2, 0, 100., false) + "%," +
			FormatUtil.FormatDouble (forwardVolatility, 2, 0, 100.) + "%," +
			FormatUtil.FormatDouble (forwardFundingCorrelation, 2, 0, 100.) + "%] =" +
			FormatUtil.FormatDouble (swapRate, 1, 4, 100.) + "% | " +
			FormatUtil.FormatDouble (swapRate - baselineSwapRate, 1, 0, 10000.)
		);
		...
		throws Exception
	{
		...
		double[] correlationForwardForward2DomXArray =
		{
			-0.99,
			-0.50,
			 0.00,
		...
		FixFloatComponent irs = CreateIRS (today.addTenor (tenor), "5Y", forwardLabel, 0.05, currency);
		...
		double baselineSwapRate = RunWithVolCorrelation (
			irs,
			valuationParams,
			curveSurfaceQuoteContainer,
			forwardLabel,
		...
		for (double sigmaForward : sigmaForwardArray) {
			for (double sigmaForward2DomX : sigmaForward2DomXArray) {
				for (double correlationForwardForward2DomX : correlationForwardForward2DomXArray) {
					RunWithVolCorrelation (
						...
						sigmaForward,
						sigmaForward2DomX,
						correlationForwardForward2DomX
					);
				}
			}
		}
