...
public class GlivenkoCantelliUniformBound
{
		...
		throws Exception
	{
		return GlivenkoCantelliUniformDeviation.Create (
			new BoundedIdempotentUnivariateRandom (binarySequenceGenerator.positiveProbability(), null, 1.),
			variateCount
		);
	}
		...
		for (int setIndex = 0; setIndex < setCount; ++setIndex) {
			dump += (0 != setIndex ? " |" : "") + FormatUtil.FormatDouble (
				new EfronSteinMetrics (
					GlivenkoCantelliFunction (binary, sampleCount),
		...
		throws Exception
	{
		...
		for (int setIndex = 0; setIndex < setCount; ++setIndex) {
			MultivariateRandom multivariateRandomFunction = GlivenkoCantelliFunction (binary, sampleCount);
		...
		throws Exception
	{
		...
		for (int setIndex = 0; setIndex < setCount; ++setIndex) {
			dump += (0 != setIndex ? " |" : "") + FormatUtil.FormatDouble (
				new EfronSteinMetrics (
					GlivenkoCantelliFunction (binarySequenceGenerator, sampleCount),
					IIDDraw (binarySequenceGenerator, sampleCount)
		...
		throws Exception
	{
		...
		for (int sampleSize : sampleSizeArray) {
			BoundedDifferencesRun (binarySequencyGenerator, sampleSize, setCount);
		}
