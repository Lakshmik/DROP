...
public class CIRFutureValueDistribution
{
		...
		throws Exception
	{
		...
		for (double t : tArray) {
			R1NonCentral r1NonCentral = r1CIRStochasticEvolver.futureValueDistribution (r0, t);

			dump = dump + " {" +
				FormatUtil.FormatDouble (r1NonCentral.parameters().degreesOfFreedom(), 1, 4, 1.) + " |" +
				FormatUtil.FormatDouble (r1NonCentral.parameters().nonCentralityParameter(), 1, 4, 1.) + "}";
		}
		...
		throws Exception
	{
		...
		for (double t : tArray) {
			...
			dump = dump + " {" + FormatUtil.FormatDouble (r1NonCentral.mean(), 1, 4, 1.) + " |" +
				FormatUtil.FormatDouble (r1NonCentral.variance(), 1, 4, 1.) + "}";
		}
		...
		throws Exception
	{
		...
		System.out.println (
			"\t|                - Chi-Square Degrees of Freedom and Non-Centrality Parameter over t                                                                ||"
		);
		...
		for (double meanReversionSpeed : meanReversionSpeedArray) {
			for (double meanReversionLevel : meanReversionLevelArray) {
				NonCentralChiSquareParameters (r0, meanReversionSpeed, meanReversionLevel, tArray);
			}
		}
		...
		System.out.println (
			"\t|                - Chi-Square Mean and Variance over t                                                                                              ||"
		);
		...
		for (double meanReversionSpeed : meanReversionSpeedArray) {
			for (double meanReversionLevel : meanReversionLevelArray) {
				NonCentralChiSquareStatistics (r0, meanReversionSpeed, meanReversionLevel, tArray);
			}
		}
