...
public class UncollateralizedCollateralGroupCorrelated
{
	...
	private static final double[] NumeraireValueRealization (
		...
		final double intialNumeraireValue,
		final double time,
		...
		throws Exception
	{
		...
		numeraireValueArray[0] = intialNumeraireValue;
		...
		JumpDiffusionEdge[] jumpDiffusionEdgeArray = numeraireValueDiffusionEvolver.incrementSequence (
			new JumpDiffusionVertex (time, intialNumeraireValue, 0., false),
		...
		throws Exception
	{
		...
		for (int vertexIndex = 0; vertexIndex < vertexNodeArray.length; ++vertexIndex) {
			dump = dump + " " + vertexNodeArray[vertexIndex] + " |";
		}
