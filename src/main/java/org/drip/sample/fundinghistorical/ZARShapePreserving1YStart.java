 ...
 *		<tr><td><b>Package</b></td> <td><a href = "https://github.com/lakshmik/DROP/tree/master/src/main/java/org/drip/sample/fundinghistorical/README.md">Smooth Shape Preserving Funding Historical</a></td></tr>
 *  </table>
 *	<br>
 * 
...
public class ZARShapePreserving1YStart
{
		...
		throws Exception
	{
		...
		for (int closeDateIndex = 0; closeDateIndex < closeDateCount; ++closeDateIndex) {
			...
			for (String inTenor : inTenorArray) {
				for (String forTenor : forTenorArray) {
					double nativeForwardRate = Double.NaN;

					try {
						nativeForwardRate = fundingCurveMetrics.nativeForwardRate (inTenor, forTenor);
					} catch (Exception e) {
					}

					dump += "," + FormatUtil.FormatDouble (nativeForwardRate, 1, 5, 100.);
				}
			}
