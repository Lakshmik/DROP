...
/**
 * <i>AntonovKonikovSpector2015</i> implements the Anotonov, Konikov, and Spector (2015) Variant of the SABR
 * 	C Function. The References are:
 *  
 * <br><br>
 *  <ul>
 *  	<li>
 *  		Antononov, A., M. Konikov, and M. Spector (2015): The Free Boundary SABR: Natural Extension to
 *  			Negative Rates <b>eSSRN</b> https://papers.ssrn.com/sol3/papers.cfm?abstract_id=2557046
 *  	</li>
 *  	<li>
 ...
 *
 *  <ul>
 * 		<li><i>AntonovKonikovSpector2015</i> Constructor</li>
...
public class AntonovKonikovSpector2015
	extends CFunction
{
	private double _beta = Double.NaN;

	/**
	 * <i>AntonovKonikovSpector2015</i> Constructor
	 * 
	 * @param beta Beta
	 * 
	...
	public AntonovKonikovSpector2015 (
		final double beta)
		throws Exception
	{
		if (!NumberUtil.IsValid (_beta = beta) || 0. > _beta || 0.5 < _beta) {
			throw new Exception ("AntonovKonikovSpector2015 Constructor => Invalid Inputs");
		}
	}
		...
		throws Exception
	{
		if (!NumberUtil.IsValid (shiftedForward)) {
			throw new Exception ("AntonovKonikovSpector2015::c => Invalid Input");
		}
		...
		return Math.pow (Math.abs (shiftedForward), _beta);
	}
		...
		throws Exception
	{
		if (!NumberUtil.IsValid (initialShiftedForward) || !NumberUtil.IsValid (shiftedStrike)) {
			throw new Exception ("AntonovKonikovSpector2015::reciprocalIntegral => Invalid Inputs");
		}

		double absoluteInitialShiftedForward = Math.abs (initialShiftedForward);

		double absoluteShiftedStrike = Math.abs (shiftedStrike);

		if (absoluteInitialShiftedForward == absoluteShiftedStrike) {
			return 0.;
		}

		if (0. == _beta) {
			return absoluteInitialShiftedForward - absoluteShiftedStrike;
		}

		if (1. == _beta) {
			return Math.log (absoluteInitialShiftedForward / absoluteShiftedStrike);
		}

		return Math.pow (absoluteInitialShiftedForward, 1. - _beta) -
			Math.pow (absoluteShiftedStrike, 1. - _beta);
	}
		...
		throws Exception
	{
		if (!NumberUtil.IsValid (shiftedForwardMid)) {
			throw new Exception ("AntonovKonikovSpector2015::gamma1 => Invalid Input");
		}

		return 0. == _beta ? 0. : _beta / Math.abs (shiftedForwardMid);
	}
	...
	{
		if (!NumberUtil.IsValid (shiftedForwardMid)) {
			throw new Exception ("AntonovKonikovSpector2015::gamma2 => Invalid Input");
		}
