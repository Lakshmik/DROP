...
public class OptimalTrajectoryWithDrift
{
		...
		throws Exception
	{
		...
		for (int timeStepIndex = 0; timeStepIndex <= n; ++timeStepIndex) {
			if (0 == timeStepIndex) {
				System.out.println (
					"\t|" + FormatUtil.FormatDouble (executionTimeNodeArray[timeStepIndex], 1, 0, 1.) +
						" => " + FormatUtil.FormatDouble (holdingsArray[timeStepIndex], 7, 1, 1.) + " | " +
				...
				System.out.println (
					"\t|" + FormatUtil.FormatDouble (executionTimeNodeArray[timeStepIndex], 1, 0, 1.) +
						" => " + FormatUtil.FormatDouble (holdingsArray[timeStepIndex], 7, 1, 1.) + " | " +
						FormatUtil.FormatDouble (tradeListArray[timeStepIndex - 1], 6, 1, 1.) + " ||"
				);
			}
		}
