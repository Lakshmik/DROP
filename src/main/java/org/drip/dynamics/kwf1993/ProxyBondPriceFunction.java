	...
	extends R1ToR1
{
	...
	private double _calibrationEndTime = Double.NaN;
	private KalotayWilliamsFabozzi _kalotayWilliamsFabozzi = null;
	private TreeMap<Double, Double> _projectedBaseForwardYieldMap = null;
	private KalotayWilliamsFabozziMarket _kalotayWilliamsFabozziMarket = null;
	 ...
	 * @param projectedBaseForwardYieldMap Projected Base Forward Yield Map
	 * @param calibrationEndTime Calibration End Time
		...
		throws Exception
	{
		...
		if (null == (_kalotayWilliamsFabozzi = kalotayWilliamsFabozzi) ||
			null == (_kalotayWilliamsFabozziMarket = kalotayWilliamsFabozziMarket) ||
			!NumberUtil.IsValid (_calibrationEndTime = calibrationEndTime) || 0. >= _calibrationEndTime ||
			null == (_underlying = underlying) || _underlying.maturityTime() < _calibrationEndTime)
		{
			throw new Exception ("ProxyBondPriceFunction Constructor => Invalid Inputs");
		}

		_projectedBaseForwardYieldMap = projectedBaseForwardYieldMap;
	}
	...
	/**
	 * Retrieve the <i>KalotayWilliamsFabozziMarket</i> Instance
	 * 
	 * @return <i>KalotayWilliamsFabozziMarket</i> Instance
	 */

	public KalotayWilliamsFabozziMarket kalotayWilliamsFabozziMarket()
	{
		return _kalotayWilliamsFabozziMarket;
	}

	/**
	 * Retrieve the Projected Base Forward Yield Map
	 * 
	 * @return Projected Base Forward Yield Map
	 */

	public TreeMap<Double, Double> projectedBaseForwardYieldMap()
	{
		return _projectedBaseForwardYieldMap;
	}

	/**
	 * Retrieve the Calibration End Time
	 * 
	 * @return Calibration End Time
	 */

	public double calibrationEndTime()
	{
		return _calibrationEndTime;
	}
	...
	/**
	 * Evaluate for the given Projected Base Forward Yield
	 * 
	 * @param projectedBaseForwardYield Projected Base Forward Yield
	 *  
	...
	@Override public double evaluate (
		...
		throws Exception
	{
		if (null == _projectedBaseForwardYieldMap) {
			_projectedBaseForwardYieldMap = new TreeMap<Double, Double>();
		}

		_projectedBaseForwardYieldMap.put (_calibrationEndTime, projectedBaseForwardYield);

		TreeMap<Double, List<Double>> projectedBondValueListMap = _underlying.value (
			_kalotayWilliamsFabozzi.tree (_kalotayWilliamsFabozziMarket, _projectedBaseForwardYieldMap)
		);

		if (null == projectedBondValueListMap) {
			throw new Exception ("ProxyBondPriceFunction::evaluate => Invalid Inputs");
		}
	...
	public double baseYieldForPrice (
		...
		throws Exception
	{
		return new FixedPointFinderBrent (price, this, true).findRoot().root();
	}
}
