...
public class ZeroStrikeCallOption
{
		...
		throws Exception
	{
		...
		MergedDiscountForwardCurve overnightCurve = (MergedDiscountForwardCurve) OvernightCurve (
			currency,
			spotDate
		);

		MergedDiscountForwardCurve fundingCurve = (MergedDiscountForwardCurve) FundingCurve (
			currency,
			spotDate
		);
		...
		for (String tenor : tenorArray) {
			...
			double fundingZeroRate = fundingCurve.zero (tenor);

			double overnightZeroRate = overnightCurve.zero (tenor);

			System.out.println (
				"\t|| " + date + " | " + FormatUtil.FormatDouble (overnightCurve.df (date), 3, 2, 100.) +
				" | " + FormatUtil.FormatDouble (fundingCurve.df (date), 3, 2, 100.) + " | " +
				FormatUtil.FormatDouble (overnightZeroRate, 1, 2, 100.) + "% | " +
				FormatUtil.FormatDouble (fundingZeroRate, 1, 2, 100.) + "% ||" +
				FormatUtil.FormatDouble (fundingZeroRate - overnightZeroRate, 3, 0, 10000.) + " ||"
			);
		}
