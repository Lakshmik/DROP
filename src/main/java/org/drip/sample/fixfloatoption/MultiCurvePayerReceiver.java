...
public class MultiCurvePayerReceiver
{
		...
		throws Exception
	{
		...
		double customMetricVolatility = 0.4;
		String manifestMeasure = "SwapRate";
		...
		FixFloatComponent stir = CreateSTIR (today.addTenor (tenor), "5Y", forwardLabel, 0.05, currency);
		...
		for (Map.Entry<String, Double> stirReceiverOptionMeasureEntry : new FixFloatEuropeanOption (
		...
		{
			System.out.println (
				"\t|| " + stirReceiverOptionMeasureEntry.getKey() + " => " +
					stirReceiverOptionMeasureEntry.getValue()
			);
		}
		...
		for (Map.Entry<String, Double> stirPayerOptionMeasureEntry : new FixFloatEuropeanOption (
				stir.name() + "::PAYER_OPT",
				stir,
				manifestMeasure,
				false,
		...
		{
			System.out.println (
				"\t|| " + stirPayerOptionMeasureEntry.getKey() + " => " +
					stirPayerOptionMeasureEntry.getValue()
			);
		}
