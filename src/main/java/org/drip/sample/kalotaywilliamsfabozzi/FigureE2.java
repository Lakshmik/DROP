...
import org.drip.dynamics.kwf1993.ProxyBondPriceFunction;
...
/**
 * <i>FigureE2</i> illustrates the Calibration and Reconciliation of Figure E in Kalotay, Williams, and
 * 	Fabozzi (1993). The References are:
 *  
 * 	<br>
 *  <ul>
 * 		<li>
...
public class FigureE2
{
	...
	private static final Map<Double, List<Double>> ProjectedBulletBondValueListMapReconciler()
	{
		...
		projected1YBulletBondValueList.add (100.000);
		...
		projected2YBulletBondValueList.add (99.926);

		projected2YBulletBondValueList.add (98.074);
		...
		projected3YBulletBondValueList.add (99.972);

		projected3YBulletBondValueList.add (99.022);

		projected3YBulletBondValueList.add (97.886);
		...
		throws Exception
	{
		...
		double bondCoupon = 0.045;
		double bondMaturityTime = 3.;
		double calibrationPrice = 1.;
		double calibrationEndTime = 3.;
		double annualizedForwardVolatility = 0.1;
		double projectedBulletBaseForwardYieldReconciler = 0.0453;
		...
		TreeMap<Double, Double> projectedBaseForwardYieldMap = ProjectedBaseForwardYieldMapInput();
		...
		double calibratedBulletBaseYield = new ProxyBondPriceFunction (
		...
		System.out.println ("\t| Kalotay, Williams, and Fabozzi (1993) Figure E2 ||");

		System.out.println ("\t|-------------------------------------------------||");

		System.out.println (
			"\t| Calibration End Time                => " +
				FormatUtil.FormatDouble (calibrationEndTime, 1, 0, 1., false)
		);

		System.out.println (
			"\t| Calibration Price                   => " +
				FormatUtil.FormatDouble (calibrationPrice, 2, 2, 100., false)
		);

		System.out.println ("\t|-------------------------------------------------||");

		System.out.println (
			"\t| Calibration Base Yield (Reconciler) => " +
				FormatUtil.FormatDouble (calibratedBulletBaseYield, 1, 3, 100., false) + "% (" +
				FormatUtil.FormatDouble (projectedBulletBaseForwardYieldReconciler, 1, 3, 100., false) + "%)"
		);

		System.out.println ("\t|-------------------------------------------------||\n");

		projectedBaseForwardYieldMap.put (calibrationEndTime, calibratedBulletBaseYield);

		KalotayWilliamsFabozziTree kalotayWilliamsFabozziTree = kalotayWilliamsFabozzi.tree (
			kalotayWilliamsFabozziMarket,
			projectedBaseForwardYieldMap
		);
